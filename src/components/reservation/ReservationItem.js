...
const ReservationItem = ({ reservation }) => {
  const {
    id, vehicle_id, reservation_date, return_date,
  } = reservation;
    ...
    <li className="group py-3 sm:py-4">
      <div className="flex flex-col sm:flex-row items-center space-x-4">
        <div className="flex-shrink-0">
          <img className="w-36" src={vehicle.pict} alt="Vehicle" />
        </div>
        ...
        <div className="flex flex-col justify-between items-center text-base font-semibold text-gray-900 dark:text-white">
          ...
          <button type="button" onClick={() => dispatch(deleteReservation(id))} className="invisible group-hover:visible">Cancel</button>
        </div>
      </div>
    </li>
  );
