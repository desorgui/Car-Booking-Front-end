...
const ShowVehicle = () => {
  const { id } = useParams();
  const vehicle = useSelector((state) => state.vehicles.find((vehicle) => vehicle.id == id)); // eslint-disable-line eqeqeq
          ...
          <section className="h-fit flex-col grow gap-8 mt-16 sm:flex sm:flex-row sm:gap-4 sm:h-full sm:mt-24 sm:mx-2 md:gap-8 md:mx-4 lg:flex-col lg:mx-0 lg:mt-24">
            <picture className="relative flex items-center bg-orange sm:bg-transparent">
              <img
                src={vehicle.pict}
                alt="sneaker"
                className="block sm:rounded-xl xl:w-[70%] xl:rounded-xl m-auto pointer-events-none transition duration-300 lg:w-3/4 lg:pointer-events-auto lg:cursor-pointer"
                id="hero"
              />
            </picture>
          </section>
