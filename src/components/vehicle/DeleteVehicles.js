      ...
      {vehicles.map((vehicle) => (
          <div className=" bg-gray-100 flex items-center" key={vehicle.id}>
            <div className="container mx-auto p-9 bg-white max-w-sm rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition duration-300">
              <img className="rounded-xl" src={jeep3} alt="" />
              <div className="flex justify-between items-center">
                ...
                <div>
                  <button onClick={() => remove(vehicle.id)} className="text-white text-md font-semibold bg-red-400 py-2 px-4 rounded-lg shadow-md hover:shadow-lg transition duration-500 transform-gpu hover:scale-110 ">Delete</button>
                </div>
              </div>
            </div>
          </div>
